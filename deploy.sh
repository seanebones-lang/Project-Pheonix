...
verify_deployment() {
    ...
    kubectl wait --for=condition=ready pod -l app=mothership-frontend -n $NAMESPACE --timeout=300s
...
show_access_info() {
    ...
    echo "📊 Prometheus: http://mothership.local:9090"
    echo "📈 Grafana: http://mothership.local:3000 (admin/admin)"
    echo ""
    ...
    echo "kubectl port-forward -n $NAMESPACE svc/grafana-service 3001:3000"
}
